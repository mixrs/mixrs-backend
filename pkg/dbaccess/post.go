...
func (e *Env) GetPosts(channelId string) ([]*models.FetchPostModel, error) {
	...
	if err := e.DB.Model(&models.ChannelModel{}).Where("channel_models.id = ?", channelId).Select("post_models.*, user_models.id as user_id, user_models.name as user_name, user_models.email as user_email, user_models.image as user_image, channel_models.id as channel_id, channel_models.title as channel_title, channel_models.description as channel_description").Joins("inner join post_models on channel_models.id = post_models.channel_id").Joins("inner join user_models on post_models.user_id = user_models.id").Scan(&res).Error; err != nil {
		log.Println(err.Error())
